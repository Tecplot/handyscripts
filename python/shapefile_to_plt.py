...
import os
import time
# Requires pyshp to be installed: https://pypi.python.org/pypi/pyshp
...
def create_connectivity_list(shape, element_offset=0):
    ...
    # Get the element indices for each shape to create the connectivity list
...
def convert_to_one_zone_per_shape(s, name_index, dataset):
    ...
    # For each shape we create a separate Tecplot zone
...
if len(sys.argv) != 3:
    print("Usage:\nshapefile_to_linedata.py someshapefile.shp outputfile.plt")
else:
